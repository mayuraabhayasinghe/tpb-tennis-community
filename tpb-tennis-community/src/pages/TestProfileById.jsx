...
const TestProfileById = () => {
  ...
  if (!allError && allRatings.length > 0) {
    const avg =
      allRatings.reduce((sum, r) => sum + r.rating, 0) / allRatings.length;
    setUserData((prev) => ({ ...prev, rating: avg.toFixed(2) }));
  }
...
{currentUserId && currentUserId !== id && (
  <div className="mt-4">
    <p className="text-gray-600 text-sm mb-1">
      {userRating ? "Update your rating:" : "Rate this player:"}
    </p>
      ...
      {[1, 2, 3, 4, 5].map((num) => (
        <button
          ...
          className={`w-8 h-8 rounded-full ${
            num <= ratingValue ? "bg-yellow-400" : "bg-gray-300"
          }`}
        >
          {num}
        </button>
...
export default TestProfileById;
